...
Now let's select a Connector. For this example, we'll use the one we built in the "Building a Connector" guide. (If you don't have any connectors yet, complete that guide, and come back here afterward.) Click the connector, and a sidebar pane will appear. The first tab of that pane says "Services". On it, you'll see at least one service. Click the purple button that says "+ Add" on the first one.
